...
export function ProfileUpdated({
  ...
  const fieldList   = updatedFields.map(f => FIELD_LABELS[f] ?? f).join(', ');

  return (
    <Html lang="en">
      <Head />
      <Preview>Your Vielink profile was updated</Preview>
      <Body style={body}>
        <Container style={container}>
          ...
          <Text style={text}>
            Your account profile was successfully updated.
          </Text>
          ...
          <Text style={detailRow}>
            <span style={detailKey}>Changed</span>
            <span style={detailVal}>{fieldList}</span>
          </Text>
          <Text style={detailRow}>
            <span style={detailKey}>Date</span>
            <span style={detailVal}>{updatedAt}</span>
          </Text>
          ...
          <Text style={text}>
            ...
            and contact support if something looks wrong.
          </Text>
...
const hr: React.CSSProperties     = { borderColor: '#eaeaea', margin: '26px 0' };
